/*
 * Rabbit Miner V4.5 - NUCLEAR MODE EDITION
 * Designed by Arez
 * 
 * Features:
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n\n=== Rabbit Miner V4.5 NUCLEAR ===");
  Serial.println("FAN-COOLED EDITION - NUCLEAR MODE ACTIVATED!");
  ...
  delay(100);
  drawMainUI();
  Serial.println("Mining UI displayed - NUCLEAR MODE ACTIVE!");
}
...
void loop() {
  ...
  // Update UI (0.1 FPS - every 10 seconds for NUCLEAR MINING!)
...
void startDualCoreMining() {
  Serial.println("Starting DUAL-CORE NUCLEAR mining!");
  
  // Create mining task on Core 0 (MAXIMUM PRIORITY & STACK!)
  xTaskCreatePinnedToCore(
    miningTaskCore0,
    "Mining0",
    20000,  // DOUBLED stack size!
    NULL,
    configMAX_PRIORITIES - 1,  // ABSOLUTE MAXIMUM PRIORITY!
    &MiningTask1,
    0
  );
  
  // Create mining task on Core 1 (MAXIMUM PRIORITY & STACK!)
  xTaskCreatePinnedToCore(
    miningTaskCore1,
    "Mining1",
    20000,  // DOUBLED stack size!
  ...
  Serial.println("Both cores mining at NUCLEAR SPEED!");
}
...
void miningTaskCore0(void * parameter) {
  ...
  Serial.println("Core 0: NUCLEAR MINING STARTED!");
  
  while (true) {
    // Batch 100 hashes for MAXIMUM throughput!
    ...
    // Minimal yield - only to feed watchdog
    taskYIELD();
  }
}
...
void miningTaskCore1(void * parameter) {
  ...
  Serial.println("Core 1: NUCLEAR MINING STARTED!");
...
void showBootScreen() {
  ...
  tft.setCursor(50, 60);
  tft.setTextColor(COLOR_YELLOW);
  tft.println("NUCLEAR EDITION!");
  
  tft.setTextSize(1);
  tft.setCursor(70, 85);
  tft.setTextColor(COLOR_GRAY);
  tft.println("Made By Arez V4.5");
  ...
  tft.println("NUCLEAR MODE ACTIVE");
