/*
 * Rabbit Miner V4.4 - BEAST MODE EDITION
 * Designed by Arez
 * 
 * Features:
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n\n=== Rabbit Miner V4.4 BEAST ===");
  Serial.println("FAN-COOLED EDITION - BEAST MODE ACTIVATED!");
  ...
  delay(100);
  drawMainUI();
  Serial.println("Mining UI displayed - BEAST MODE ACTIVE!");
}
...
void loop() {
  ...
  // Update UI (0.2 FPS - every 5 seconds for MAX MINING!)
...
void startDualCoreMining() {
  Serial.println("Starting DUAL-CORE BEAST mining!");
  
  // Create mining task on Core 0 (PRIORITY 3 - BEAST MODE!)
  ...
  // Create mining task on Core 1 (PRIORITY 3 - BEAST MODE!)
  xTaskCreatePinnedToCore(
    miningTaskCore1,
    "Mining1",
    10000,
  ...
  Serial.println("Both cores mining at BEAST SPEED!");
}
...
void miningTaskCore0(void * parameter) {
  ...
  Serial.println("Core 0: BEAST MINING STARTED!");
  
  while (true) {
    // Batch 50 hashes then yield (optimal balance!)
    ...
    // Minimal yield to feed watchdog
...
void miningTaskCore1(void * parameter) {
  ...
  Serial.println("Core 1: BEAST MINING STARTED!");
...
void showBootScreen() {
  ...
  tft.setCursor(50, 60);
  tft.setTextColor(COLOR_YELLOW);
  tft.println("BEAST EDITION!");
  
  tft.setTextSize(1);
  tft.setCursor(70, 85);
  tft.setTextColor(COLOR_GRAY);
  tft.println("Made By Arez V4.4");
